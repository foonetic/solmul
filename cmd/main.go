...
func mainFunc(cmd *cobra.Command, args []string) {
	...
	for _, a_url := range urls {
		if endpoint, err := solmul.GetEndpoint(a_url); err == nil {
...
`,
	...
	rootCmd.Flags().StringArrayVarP(&urls, "url", "u", []string{}, "URLs for Solana RPC. Can be a URL like http://127.0.0.1:8899 or mainnet-beta|devnet|testnet|mainnet-beta-serum")
